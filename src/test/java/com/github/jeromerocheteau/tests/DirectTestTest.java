...
public class DirectTestTest {

	private Templater templater;
	
    @Before
    public void setUp() throws Exception {
    	InputStream stream = this.getClass().getResourceAsStream("/direct-test-test.template");
	...
	public void doBothUndefined() throws Exception {
		...
		Assert.assertEquals("!", buffer.toString().trim());
	}
	...
	public void doFirstEmpty() throws Exception {
		...
		scope.put("message", message);
		StringBuffer buffer = templater.doProcess(scope);
		Assert.assertEquals("World!", buffer.toString().trim());
	}
	...
	public void doLastEmpty() throws Exception {
		...
		scope.put("message", message);
		StringBuffer buffer = templater.doProcess(scope);
		Assert.assertEquals("Salut !", buffer.toString().trim());
	}
